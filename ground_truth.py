...
# Parameters 
bler_target_list = [0.1] # 1e-6  
b = len(bler_target_list)    # target BLER 0.000001
n_slots = 1000        # total TTI
...
bler_hist = np.full((b, n_slots), np.nan)
se_value_hist = np.full((b, n_slots), np.nan)
...
for b in range(len(bler_target_list)):
    ...
    for t in range(n_slots):
        ...
        if _df_bler_verified.empty:
            ...
            bler_hist[b][t] = _df_se_verified["BLER"].iloc[0]
            #bler_hist[b][t] = 0
            #print(f"tti:{tti_hist[t]}, sinr:{sinr_true[t]}, mcs:{mcs_hist[b][t]}")
        ...
        se_value_hist[b][t] = (1-bler_hist[b][t])*(_df_se_verified["SE"].iloc[0])
        #print(f"sinr:{sinr_true[t]}, mcs:{mcs_hist[b][t]}, bler:{bler_hist[b][t]}, se_value:{se_value_hist[b][t]}")
...
for b_idx, b in enumerate(bler_target_list):
    ax2.boxplot(mcs_hist[b_idx])
ax2.grid()
ax2.set_title("box plot selected mcs")
#ax2.legend()

# for b_idx, b in enumerate(bler_target_list):
#     ax3.plot(tti_hist, se_value_hist[b_idx], label=f"bler_target={b}")
...
# ecdf bler
for b_idx, b in enumerate(bler_target_list):
    ax4.ecdf(bler_hist[b_idx], label=f"bler_target={b}")
ax4.grid()
